...
void main() {
    ...
    vNormal = normalMatrix * rose_normal(position, a, b, c, d, tdelta, pdelta);

    // Pass world position for fresnel calculation
    vWorldPosition = (modelMatrix * vec4(rosePosition, 1.0)).xyz;
...
void main() {
    ...
    // Refraction vectors with chromatic aberration
...
void main() {
    // Use the sphere geometry's normal directly
    ...
    // Use position directly from sphere geometry
    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
}`;

// Fragment shader remains mostly the same but with adjusted parameters
const fragmentModelShader = `
uniform vec3 chromaticDispersion;
uniform float bias;
uniform float scale;
